		...
		{			
			...
			if (!RayCastUtils.getNearbyEntities(p, 20, target ->
			{
				...
				if (GlacioEffects.chillEffects.containsKey(target.getUniqueId()))
				{
					...
					MobData.damageUnnaturally(p, target, damage*3, false, true, ElementType.GLACIO, null);
				}
				GlacioEffects.addFrozen(target);
				MobData.damageUnnaturally(p, target, 10, false, true, ElementType.GLACIO, null);
			})) return -1;
