 ...
 */
public record EchoManifest(String echoId, Rarity rarity, EchoData baseStats, List<WeaponModifier> modifiers, ElementiumSlotType slotType)
{
    /**
    ...
    /**
     * Returns true if this Echo has an Elementium Slot available for ability equipment.
     */
    ...
    {
        return modifiers.stream().anyMatch(WeaponModifier::isActive);
    }

    /**
     * Filters and returns only the active modifiers on this manifest.
     */
    public List<WeaponModifier> getActiveModifiers()
    {
        return modifiers.stream().filter(WeaponModifier::isActive).toList();
    }

    /**
     * Filters and returns only the passive modifiers on this manifest.
     */
    public List<WeaponModifier> getPassiveModifiers()
    {
